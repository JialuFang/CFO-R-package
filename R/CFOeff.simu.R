#' Conduct one simulation using the calibration-free odds (CFO) for phase I/II trials
#'
...
#'                   represent the parameters of the Jeffreys' prior distribution for the true efficacy rate at any dose level.This prior distribution 
...
#' @param seed a vector of random seeds for each simulation.
...
#'         \item neff: the total number of OBD selection for all dose levels.
...
CFOeff.simu <- function(target, p.true, pE.true, ncohort=10, init.level=1, cohortsize=3,
  ...
  for (i in 1:ncohort) {
    ...
    cidx <- CFOeff.next(target, txs, tys, tns, cidx, prior.para, cutoff.eli, early.stop, effearly.stop, mineff)$nextdose
  ...
  out <- list(OBD = OBD, target = target, npatients = tns, neff = txs, ntox = tys, pE.true = pE.true, p.true = p.true, 
              ...
              sumDLT = sum(DLTlist), sumeff = sum(EFFlist), earlystop = earlystop, class = "phaseI/II")
