...
#' @param seeds A vector of random seeds for each simulation, for example, \code{seeds = 1:n_sim} (default is NULL).
#'
...
#'         tau, accrual, tite.dist, accrual.dist, add.args, seeds = 1:nsimu)
...
CFO.oc <- function(nsimu=5000, design, phi, p.true, ncohort, init.level, cohortsize,
  ...
  run.fn <- function(i){
    if (design == 'CFO' || design == 'aCFO'){
      res <- CFO.simu(phi, p.true, ncohort, init.level, cohortsize, design, add.args, seed = seeds[i])
    }else{
      res <- lateonset.simu(phi, p.true, tau, cohortsize, ncohort, accrual, tite.dist, accrual.dist, 
                            design, init.level, add.args=list(alp.prior=phi, bet.prior=1-phi), seed = seeds[i])
    }
    ress <- list(
      res=res,
