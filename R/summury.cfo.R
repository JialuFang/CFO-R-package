...
#' decision <- lateonset.next(target, p.true, currdose=4, design='faCFO', tau=3, enter.times, dlt.times,  
  ...
  if(!is.null(object$correct)){ ###summary for XXX.simu()
    if (length(object$MTD) == 1) {  ###summary for one-dim XXX.simu()
      if (object$MTD == 99) {
        ...
        cat("For",length(object$patientDLT),"cohorts, the dose level assigned to each cohort is: \n")
        cat(formatC(object$patientDLT, format = "d"), sep = "  ", "\n")
